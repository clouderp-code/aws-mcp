...
# Call Analysis MCP Server - Test Script for analyze-transcript-batch tool
# This script tests the analyze-transcript-batch endpoint with realistic parameters

set -e

# Configuration
SERVER_URL="http://localhost:8000"
TOOL_ENDPOINT="$SERVER_URL/tools/analyze-transcript-batch"
...
print_header() {
    ...
    echo -e "${PURPLE}${BOLD}📊 Test Script: analyze-transcript-batch Tool${NC}"
    echo -e "${PURPLE}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
}
...
test_basic_functionality() {
    echo -e "${BLUE}🧪 Testing basic analyze-transcript-batch functionality...${NC}"
...
test_with_analysis_options() {
    ...
    local test_data='{
        "s3_bucket": "call-transcripts-demo",
        "s3_prefix": "transcripts/batch-002/",
        "output_bucket": "call-analysis-results",
        "output_prefix": "batch-analysis-002",
        "max_files": 5,
        "analysis_options": {
...
test_large_batch() {
    echo -e "${BLUE}🧪 Testing large batch processing...${NC}"
    
    # Test data for larger batch
    local test_data='{
        "s3_bucket": "call-transcripts-demo",
        "s3_prefix": "transcripts/large-batch/",
        "output_bucket": "call-analysis-results",
        "output_prefix": "large-batch-analysis",
        "max_files": 50,
        "aws_region": "us-east-1"
    }'
    
    echo -e "${CYAN}📤 Sending request for large batch processing...${NC}"
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}📥 Response received:${NC}"
        echo "$response" | jq '.' 2>/dev/null || echo "$response"
        print_success "Large batch test completed"
    else
        print_error "Request failed"
        return 1
    fi
...
test_invalid_parameters() {
    ...
    # Test with invalid max_files value
    local test_data='{
        "s3_bucket": "call-transcripts-demo",
        "s3_prefix": "transcripts/batch-003/",
        "output_bucket": "call-analysis-results",
        "max_files": -1
    }'
    
    echo -e "${CYAN}📤 Testing with invalid max_files value...${NC}"
...
show_usage() {
    ...
    echo -e "  --large-batch        Run large batch processing test"
    ...
    echo -e "  $0 --large-batch        # Test large batch processing"
...
main() {
    ...
    if [ "$run_basic" = false ] && [ "$run_analysis_options" = false ] && [ "$run_large_batch" = false ] && [ "$run_error_handling" = false ] && [ "$run_invalid_params" = false ]; then
        run_basic=true
        run_analysis_options=true
        run_large_batch=true
        run_error_handling=true
    ...
    print_header
    print_success "All tests completed for analyze-transcript-batch tool"
    echo -e "${CYAN}📊 Tool: analyze-transcript-batch${NC}"
    echo -e "${CYAN}🔗 Endpoint: $TOOL_ENDPOINT${NC}"
    echo -e "${CYAN}📝 Description: Analyze multiple call transcripts from an S3 folder and generate AI-powered batch analysis report${NC}"
}
