...
# TMF ODA Transformer MCP Server - Fully Dynamic Tool Listing Script
...
show_usage() {
    echo -e "${CYAN}${BOLD}TMF ODA Transformer MCP Server - Tool Listing Script${NC}"
    echo ""
...
display_tools() {
    ...
    echo -e "${CYAN}🔧 TMF ODA Transformer Tools (${tool_count} tools discovered from server):${NC}"
    echo ""
        ...
        if [ -n "$tool_name" ]; then
            ...
            case "$tool_name" in
                "ask-analysis-question")
                    test_data='{"question": "What are the key insights from the analysis?"}'
                    ;;
                "analyze-transcript"|"analyze-transcript-batch")
                    test_data='{"s3_bucket": "test-bucket", "s3_key": "test-key", "output_bucket": "test-output"}'
                    ;;
                "read-s3-transcript")
                    test_data='{"s3_bucket": "test-bucket", "s3_key": "test-key"}'
                    ;;
                "upload-to-s3")
                    test_data='{"content": "test content", "s3_bucket": "test-bucket", "s3_key": "test-key"}'
                    ;;
                "generate-report")
                    test_data='{"analysis_result": {"test": "data"}}'
                    ;;
                "create-dashboard")
                    test_data='{"analysis_data": [{"test": "data"}]}'
                    ;;
                "generate-business-intelligence")
                    test_data='{"analysis_s3_urls": ["s3://test-bucket/test.json"], "output_bucket": "test-output"}'
                    ;;
            esac
...
main() {
    ...
    echo -e "${BLUE}🚀 Fully Dynamic TMF ODA Transformer Tool Discovery${NC}"
