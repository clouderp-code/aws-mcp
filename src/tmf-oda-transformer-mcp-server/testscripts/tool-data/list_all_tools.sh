...
show_usage() {
    ...
    echo -e "  $0 --host example.com --port 8080 # List tools on remote server"
    echo ""
}
...
test_tool_endpoints() {
    ...
    jq -r '.tools[].name' /tmp/mcp_tools.json 2>/dev/null | while read -r tool_name; do
        if [ -n "$tool_name" ]; then
            ...
            if timeout 5 curl -s --connect-timeout 2 --max-time 3 "$TOOLS_ENDPOINT/$tool_name" > /dev/null 2>&1; then
