...
# Import TransformationUtils and AWS utilities for DynamoDB-based journey management
try:
...
async def get_job_logs_tool(
    ...
    try:
        ...
        if role_arn and create_aws_client:
        ...
        try:
            ...
            # Check if this is a real response or a mock object
            try:
                logs_content = response['Body'].read().decode('utf-8')
            except (TypeError, KeyError) as access_error:
                # This might be a mock object or invalid response - treat as not found
                if 'Mock' in str(type(response)) or 'not subscriptable' in str(access_error):
                    raise ClientError(
            ...
            try:
                ...
                # JSON parsing failed - re-raise so outer handler catches it
                raise Exception(f"Invalid JSON content in logs: {str(json_error)}")
            
            # Get object metadata - handle both datetime and string for LastModified
            last_modified = None
            ...
            # Handle mock NoSuchKey exceptions from tests
            ...
            elif 'NoSuchKey' in str(e) or 'Key not found' in str(e):
                # Handle mock exceptions that mention NoSuchKey
                logger.warning(f'Logs not found for {step_name} - step may not have executed')
        ...
        # Only log and raise if we haven't already handled it above
        if not str(e).startswith('Error accessing S3 logs:'):
            error_msg = f'Failed to retrieve job logs: {str(e)}'
            logger.error(error_msg)
            await ctx.error(error_msg)
            raise Exception(error_msg)
        else:
            # Re-raise the original exception without double logging
            raise
