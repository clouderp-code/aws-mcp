...
class AWSClientManager:
    ...
    def __init__(self, role_arn: Optional[str] = None, region_name: Optional[str] = None):
        ...
        Args:
            role_arn: Optional role ARN to assume (overrides env/config)
            region_name: Optional AWS region name (overrides env/config)
        """
    ...
    def _test_instance_role(self) -> bool:
        ...
        Returns:
            True if instance role is available, False otherwise
        """
        try:
    ...
    def _create_session(self) -> boto3.Session:
        ...
        Credential priority:
        1. Instance role (if available and no role ARN specified)
        2. Role ARN assumption (if specified)
        3. Default credential chain (profiles, env vars, etc.)
